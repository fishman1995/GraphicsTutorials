...
void Renderer::RenderScene() {
	...
	if (usingStencil) {
		...
		glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE);	//关闭每个通道的颜色写入--有4个参数，红、绿、蓝和阿尔法各一个。
		glStencilFunc(GL_ALWAYS, 2, ~0);	//把数值2写入stencil缓冲区
		...
		glStencilFunc(GL_EQUAL, 2, ~0);	//模版测试被设置为只通过模版值为2的片段
		glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);	//将模版函数设置为保留内容，无论片段通过还是失败
	}
